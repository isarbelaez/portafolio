  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 md:py-24 space-y-24">
      ...
      <section className="space-y-6">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-outfit font-bold text-white tracking-tight max-w-3xl">
          Estrategias probadas para ganar <span className="text-primary italic font-light">tracción</span> y escalar.
        </h1>
        <p className="text-lg md:text-xl text-on-surface-variant font-inter font-light max-w-2xl leading-relaxed">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel p-10 rounded-3xl col-span-1">
          <h4 className="font-space font-semibold text-outline-variant uppercase text-sm tracking-widest mb-4">Proyectos Completados</h4>
          <span className="text-8xl font-bold font-space text-white tracking-tighter">29</span>
        </div>
        <div className="glass-panel p-10 rounded-3xl col-span-1 bg-gradient-to-br from-primary/10 to-transparent border-primary/20">
          ...
          <span className="text-8xl font-bold font-space text-white tracking-tighter">3.8<span className="text-5xl text-primary ml-1">x</span></span>
        </div>
        <div className="glass-panel p-10 rounded-3xl col-span-1">
          <h4 className="font-space font-semibold text-outline-variant uppercase text-sm tracking-widest mb-4">Clientes Satisfechos</h4>
          <span className="text-8xl font-bold font-space text-white tracking-tighter">92<span className="text-5xl text-on-surface-variant ml-1">%</span></span>
        </div>
      </section>
        ...
        <div className="flex w-max">
          ...
          >
            {SCROLL_ITEMS.map((item, id) => (
              <span key={id} className="text-transparent bg-clip-text bg-gradient-to-r from-on-surface-variant/30 to-outline-variant/30 whitespace-nowrap">
                {item}
              </span>
            ))}
          </motion.div>
      ...
      <section className="max-w-4xl glass-panel p-8 md:p-12 rounded-3xl">
        <h2 className="text-2xl font-outfit text-white mb-6 font-semibold">Tecnología al servicio de las ventas</h2>
        <p className="text-on-surface-variant font-inter font-light leading-relaxed mb-6">
